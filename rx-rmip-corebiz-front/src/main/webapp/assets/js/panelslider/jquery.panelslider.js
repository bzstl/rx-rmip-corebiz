...
(function($) {
	...
	var TRANSITION_END = [ 'transitionend', 'webkitTransitionEnd',
			'oTransitionEnd', 'MSTransitionEnd' ].join(' ');
	...
	$.panelslider.close = function(callback) {
		var active = $('.ps-active-panel'), options = active.data('ps-options');
		...
		$('body').removeClass(options.bodyClass).one(TRANSITION_END,
				function(e) {
					sliding = false;
					...
					if (callback) {
						// HACK: Prevent google chrome to invoke the callback
						// prematurally.
						setTimeout(function() {
							callback();
						}, 0);
	...
	$.fn.panelslider = function(options) {
		var defaults = {
			bodyClass : 'ps-active', // Class to be added to body when panel
			// is opened
			clickClose : true, // If true closes panel when clicking outside it
			onOpen : null
		// Callback after the panel opens
		};
		...
		this.click(function(e) {
			...
			if (!active.length) {
				...
				// If another panel is active, close it before opening the
				// target.
			} else {
